<?php
	...
	if(!empty($pro_plugins)) {
		?>
		<h4 class="recomplug-title"><?php echo esc_html__('Pro Plugins', 'ripple'); ?></h4>
		<p><?php echo esc_html__('Take Advantage of some of our Premium Plugins', 'ripple'); ?></p>
		<div class="recomended-plugin-wrap">
		<?php
		foreach($pro_plugins as $plugin) {

			$status = $this->ripple_plugin_active($plugin);
			$icon_url = $plugin['screenshot'];
			switch($status) {
				case 'install' :
					$btn_class = 'install-offline button';
					$label = esc_html__('Install and Activate', 'ripple');
					$link = $plugin['location'];
					break;

				case 'inactive' :
					$btn_class = 'button';
					$label = esc_html__('Deactivate', 'ripple');
					$link = admin_url('plugins.php');
					break;

				case 'active' :
					$btn_class = 'activate-offline button button-primary';
					$label = esc_html__('Activate', 'ripple');
					$link = $plugin['location'];
					break;
			}
	...
	if(!empty($free_plugins)) {
		?>
		<h4 class="recomplug-title"><?php echo esc_html__('Free Plugins', 'ripple'); ?></h4>
		<p><?php echo esc_html__('These Free Plugins might be handy for you.', 'ripple'); ?></p>
		<div class="recomended-plugin-wrap">
		<?php
		foreach($free_plugins as $plugin) {
			$info = $this->ripple_call_plugin_api($plugin['slug']);

			$icon_url = $this->ripple_check_for_icon($info->icons);
			$status = $this->ripple_plugin_active($plugin);
			$btn_url = $this->ripple_plugin_generate_url($status, $plugin);
