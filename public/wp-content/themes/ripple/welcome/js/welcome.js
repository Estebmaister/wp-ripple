...
jQuery(document).ready(function ($) {
	...
	$(".install").on('click', function (e) {
    	...
    	var ajaxurl = RippleWelcomeObject.ajaxurl;

		$.ajax({
			type: 'POST',
			url: ajaxurl,
			data: {
				action: 'ripple_plugin_installer',
				plugin: plugin,
				plugin_file: plugin_file,
				nonce: RippleWelcomeObject.admin_nonce,
			},
			success: function(response) {
		   		...
		   		if(response == 'success'){
				   		...
				   		el.html(RippleWelcomeObject.installed_btn);
	...
	$('.install-offline').on('click', function (e) {
		...
		$.ajax({
			...
			url: ajaxurl,
			data: {
				action: 'ripple_plugin_offline_installer',
	...
	$(".activate").on('click', function (e) {
		...
		$.ajax({
	   		...
	   		url: ajaxurl,
	   		data: {
	   			action: 'ripple_plugin_activation',
	   			plugin: plugin,
	   			nonce: RippleWelcomeObject.activate_nonce,
	   			dataType: 'json'
	   		},
	   		success: function(response) {
		   		if(response){
	...
	$('.activate-offline').on('click', function (e) {
		...
		$.ajax({
	   		...
	   		url: ajaxurl,
	   		data: {
	   			action: 'ripple_plugin_offline_activation',
	   			plugin: plugin,
	   			nonce: RippleWelcomeObject.activate_nonce,
	   			dataType: 'json'
	   		},
