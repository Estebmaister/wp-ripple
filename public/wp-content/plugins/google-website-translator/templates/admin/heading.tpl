<div class="prisna_gwt_section prisna_gwt_{{ type }}{{ value.false:begin }} prisna_gwt_heading_hiding{{ value.false:end }}{{ dependence.show.false:begin }} prisna_gwt_no_display{{ dependence.show.false:end }}{{ has_dependence.true:begin }} prisna_gwt_section_tabbed_{{ dependence_count }}{{ has_dependence.true:end }}" id="section_{{ id }}">
{{ description.true:begin }}
	<div class="prisna_gwt_tooltip"></div>
	<div class="prisna_gwt_description prisna_gwt_no_display">{{ description_message }}</div>
{{ description.true:end }}
	<div class="prisna_gwt_title_container prisna_gwt_icon prisna_gwt_icon_grid2"><h3 class="prisna_gwt_title">{{ title_message }}</h3></div>

	<div class="prisna_gwt_clear"></div>

</div>
